...
public delegate void FreeParallaxElementRepositionLogicFunction(Parallax p, FreeParallaxElement element, float amount, GameObject obj, Renderer r);
...
public class FreeParallaxElement
{
    ...
    public void SetupState(Parallax p, Camera c, int index)
    {
    ...
    public void SetupScale(Parallax p, Camera c, int index)
    {
    ...
    public void SetupPosition(Parallax p, Camera c, int index)
    {
        ...
        for (int i = 0; i < GameObjects.Count; i++)
        {
                ...
                RepositionLogic.PositionMode == FreeParallaxPositionMode.IndividualStartOnScreen)
            {
                ...
                Parallax.SetPosition(obj, r, x, y);
            }
            else
            {
                ...
                if (RepositionLogic.PositionMode == FreeParallaxPositionMode.WrapAnchorTop)
                {
                    if (p.IsHorizontal)
                    {
                        ...
                        Parallax.SetPosition(obj, r, offset, topWorld.y - r.bounds.size.y);
                    }
                    else
                    {
                        Vector3 topWorld = c.ViewportToWorldPoint(new Vector3(1.0f, 0.0f, 0.0f));
                        Parallax.SetPosition(obj, r, topWorld.x - r.bounds.size.x, offset + r.bounds.size.y);
                    }
                }
                else if (RepositionLogic.PositionMode == FreeParallaxPositionMode.WrapAnchorBottom)
                {
                    if (p.IsHorizontal)
                    {
                        Parallax.SetPosition(obj, r, offset, screenLeft.y);
                    }
                    else
                    {
                        Parallax.SetPosition(obj, r, screenLeft.x, offset);
                    }
                }
                else
                {
                    ...
                    if (p.IsHorizontal)
                    {
                        Parallax.SetPosition(obj, r, offset, r.bounds.min.y);
                    }
                    else
                    {
                        Parallax.SetPosition(obj, r, r.bounds.min.x, offset);
                    }
                }
    ...
    public void Update(Parallax p, float t, Camera c)
    {
            ...
            else if (p.IsHorizontal)
            {
                if (t > 0 && (screenEdge.y >= c.rect.height || screenEdgeValue >= maxEdge))
                {
                    ...
                    else
                    {
                        ...
                        Parallax.SetPosition(obj, r, leftEdge.x - newWorldPoint.x, newWorldPoint.y);
                    }
                }
                else if (t < 0 && (screenEdge.y >= c.rect.height || screenEdge.x < minEdge))
                {
                    ...
                    else
                    {
                        ...
                        float randY = UnityEngine.Random.Range(RepositionLogic.MinYPercent, RepositionLogic.MaxYPercent);
                        Vector3 newWorldPoint = c.ViewportToWorldPoint(new Vector3(randX, randY));
                        Parallax.SetPosition(obj, r, rightEdge.x + newWorldPoint.x, newWorldPoint.y);
                    }
                }
            }
            else
            {
                if (t > 0 && (screenEdge.x >= c.rect.width || screenEdgeValue >= maxEdge))
                {
                    ...
                    else
                    {
                        ...
                        float randY = UnityEngine.Random.Range(RepositionLogic.MinYPercent, RepositionLogic.MaxYPercent);
                        Vector3 newWorldPoint = c.ViewportToWorldPoint(new Vector3(randX, randY));
                        Parallax.SetPosition(obj, r, newWorldPoint.x, bottomEdge.y - newWorldPoint.y);
                    }
                }
                else if (t < 0 && (screenEdge.x >= c.rect.width || screenEdge.y < minEdge))
                {
                    ...
                    else
                    {
                        ...
                        float randY = UnityEngine.Random.Range(RepositionLogic.MinYPercent, RepositionLogic.MaxYPercent);
                        Vector3 newWorldPoint = c.ViewportToWorldPoint(new Vector3(randX, randY));
                        Parallax.SetPosition(obj, r, newWorldPoint.x, topEdge.y + newWorldPoint.y);
                    }
                }
            }
        }
